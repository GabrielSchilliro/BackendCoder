...
async function handleMessages(messages) {
    const authorsSchema = new normalizr.schema.Entity('author');
    const messageSchema = new normalizr.schema.Entity('message', { author: authorsSchema });
    const messagesToShow = normalizr.denormalize(messages.result, [messageSchema], messages.entities)
    const recursoRemoto = await fetch(`views/messages.ejs`);
    const layoutText = await recursoRemoto.text();
    const functionTemplate = ejs.compile(layoutText);
    const html = functionTemplate({ messagesToShow });
